...
module.exports.getUser = async (req, res) => {
  ...
  const user = await Model.findOne({ email: googleUser.email })
    .then((u) => {
      if (u === null) {
        const googleId = googleUser.id;
        delete googleUser.id;
        u = Model.create({ ...googleUser, googleId: googleId });
      }
      return u;
    })
    .catch((err) => {});
  const token = await jwt.sign(user.toObject(), JWT_SECRET, { expiresIn: "1d" });
  res.redirect(
    `${REDIRECT_URI_FRONT}?${new URLSearchParams({
      auth: "google",
